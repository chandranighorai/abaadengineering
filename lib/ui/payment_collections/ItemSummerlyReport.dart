...
class ItemSummerlyReport extends StatefulWidget {
  final SummeryList itemMonthlyReport;

  const ItemSummerlyReport({this.itemMonthlyReport, Key key}) : super(key: key);

  @override
  _ItemSummerlyReportState createState() => _ItemSummerlyReportState();
}

class _ItemSummerlyReportState extends State<ItemSummerlyReport> {
  bool _showProgress = false;
  int _progress = 0;
  @override
  Widget build(BuildContext context) {
    SummeryList itemMonthReport = widget.itemMonthlyReport;
    var mExt = itemMonthReport.reportFiles
        .split(".")[itemMonthReport.reportFiles.split(".").length - 1];
    //print("ppp..." + widget.itemMonthlyReport.projectName.toString());
    return InkWell(
      onTap: () {
        print("....." + itemMonthReport.reportName.toString());
  ...
  void downloadFile(BuildContext context, String drawingImage) async {
    ...
    var request = new http.Request('GET', Uri.parse(drawingImage));
    var response = httpClient.send(request);
    // Directory tempDir = await getApplicationDocumentsDirectory();
    // String dir = tempDir.path;
    var imageName = drawingImage.split("/")[drawingImage.split("/").length - 1];
